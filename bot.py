...
async def menu_cmd(client, message):
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📝 Chatda Cro(sözü hər tapam oyunçu yeni aparıcı olur)", callback_data="sel_chat"),
         InlineKeyboardButton("🎤 Səslidə Cro(aparıcı sadəcə bir nəfər olur)", callback_data="sel_voice")]
    ])
