...
async def queries(client, callback_query: CallbackQuery):
    ...
    if data == "help_menu":
        ...
        if cid in active_games and active_games[cid]['apariçi'] is None:
            ...
            kb = InlineKeyboardMarkup([[InlineKeyboardButton("Sözə Bax 🔍", callback_data=f"look_{w}")],                [InlineKeyboardButton("İmtina ❌", callback_data="imtina")],
                [InlineKeyboardButton("Növbeti ♻️", callback_data=f"next_{c}_{m}")]
            ])
        ...
        await callback_query.edit_message_text("🎮 Hansı Modda oyunu başlatmaq istəyirsiniz ?", reply_markup=kb)
